...
class Target(Component):
    """
    Target of the energy Grid. Contains the sum of generation and Demand to be reached by optimizing the schedule.

    args:
        name (string): Name for internal reference
        time_series (dict): Contains the required power per time step in timestep: power dict
        electricity_prices (list): Cost of buying/selling electricity in euro/kWh to calculate the net income.
    """
        ...
        self.constraints = [
            (
                ...
                lambda model, t: (
                 ...
                 if t in time_series else model.get_attribute(self, "difference")[t]
                    == 0.0),
            ),
            (
                "abs_difference_max",
