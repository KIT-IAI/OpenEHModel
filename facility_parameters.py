...
hp_parameters = {
    "input": {
        "X_set_HP": 0,                # "Set point partial load [0,1]";
        "P_HP_last": 0,               # "Partial load last run [0,1]";
    },
    "output": {
        "Q_HP": 0,                    # "Heat output  [W]";
        "P_el": 0,                    # "Electrical input power [W] (for monitoring/EMS)";
    },
    "metadata": {
        # Reworked heat pump parameters (more realistic defaults)
        # Typical installed heat pumps for district/building heating are
        # in the 100's kW up to a few MW. This configuration uses a
        # moderate sized unit with 500 kW nominal heat output.
        "Q_nominal": 500_000,        # Nominal heat output [W]
        # Electrical input at nominal load computed from COP (P_el = Q/COP)
        "COP_HP": 3.5,               # Default coefficient of performance (can be overridden)
        "P_el_nominal": int(500_000 / 3.5),
        "P_max": 500_000,            # Max heat output [W] (500 kW)
        "P_min": 50_000,             # Min heat output [W] (10% of nominal = 50 kW)
        # Startup time: realistic ramp/soft-start for compressors/control
        "t_startup": 300,            # Startup time [s] (5 minutes)
        # Small auxiliary efficiency / losses (informational)
        "aux_eta": 0.98,
        "name": "HP",
        # EMS metadata so the heat pump can participate in optimization
        "EMS": {
            # EMS expects power bounds in Watts for the heat output
            "P_min": 50000,
            "P_max": 500000,
            # Ramp rate describes how fast the device can change power [W/s]
            "Ramp_Rate": 2000,
            "name": "HP",
            "Opt_Range": [0, 1],
            "steering": True,
            # Output names exposed to the EMS - include heat and electrical input
            "Outputs": ["Q_HP", "P_el"],
            "type": "Converter"
        },
    }
}
facility_dict[hp_parameters["metadata"]["name"]] = hp_parameters
...
bat_parameters = {
    "input": {
        ...
        "P_max_Bat": 1_000_000,         #  "Max Power (Charge, Discharge) bat [W] ";
        "EBat": 10_800_000_000,         # "Capacity bat [J] ";
        "eta_Bat": 0.93                 # efficiency of bat [0,1]
    },
    "output": {
...
facility_dict[bat_parameters["metadata"]["name"]] = bat_parameters
